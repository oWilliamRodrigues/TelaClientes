...
namespace Europa.Resources
{
    ...
    /// project.
    /// </remarks>
    public class ResourcesEnumConverter : EnumConverter
    {
        ...
        private string GetValueText(CultureInfo culture, object value)
        {
            ...
            string resourceName = $"{type.Name}_{value.ToString()}";
        ...
        public ResourcesEnumConverter(Type type, System.Resources.ResourceManager resourceManager)
            : base(type)
        {
        ...
        static public List<KeyValuePair<int, string>> GetNumberValues(Type enumType, CultureInfo culture)
        {
            List<KeyValuePair<int, string>> result = new List<KeyValuePair<int, string>>();
            TypeConverter converter = TypeDescriptor.GetConverter(enumType);
            foreach (Enum value in Enum.GetValues(enumType))
            {
                KeyValuePair<int, string> pair = new KeyValuePair<int, string>(value.GetHashCode(), converter.ConvertToString(null, culture, value));
                result.Add(pair);
            }
            return result;
        }

        static public List<KeyValuePair<int, string>> GetNumberValues(Type enumType)
        {
            return GetNumberValues(enumType, CultureInfo.CurrentUICulture);
        }
    }
}
